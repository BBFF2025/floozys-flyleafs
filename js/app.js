// /js/app.js — face-out cards, leaning spines, date precision support
...
console.log("FF app.js v5 loaded");
...
async function init(){
  ...
  // Date precision switcher (Day / Month / Year)
...
function setupEvents(){
  ...
  addForm.addEventListener('submit', async (e)=>{
    ...
    if (precision === 'month') {
      const m = (fd.get('dateReadMonth')||'').toString(); // "YYYY-MM"
      dateRead = m ? m + '-01' : '';
    }
    if (precision === 'year') {
      const y = (fd.get('dateReadYear')||'').toString().trim(); // "YYYY"
      dateRead = y ? y + '-01-01' : '';
    }
    ...
    await addBook(book);
    addForm.reset();
    // reset date precision UI to default (day)
    const datePrecSel = document.getElementById('datePrecision');
    if(datePrecSel){ datePrecSel.value='day'; datePrecSel.dispatchEvent(new Event('change')); }
    await refreshBooks();
  });
}
...
function renderBook(b, j=0){
  ...
  if(b.facing==='cover' && (b.cover||'').trim()!=='' ){
    ...
    // INDEX CARD — only when face-out; no icons; click to edit
    ...
    // use cover art as texture if available
    if ((b.cover || '').trim() !== '') {
    ...
    // subtle alternating lean & touching spines
...
function demoBooks(){
  return [
    ...
    { id:'b_2', title:'Thinking, Fast and Slow', author:'Daniel Kahneman', type:'nonfiction', rating:4, dateRead:'2022-11-02', cover:'https://covers.openlibrary.org/b/id/6917225-L.jpg', note:'Changed how I interpret my own brain glitches.', facing:'spine' },
    ...
    { id:'b_4', title:'Atomic Habits', author:'James Clear', type:'nonfiction', rating:4, dateRead:'2020-09-13', cover:'https://covers.openlibrary.org/b/id/9250116-L.jpg', note:'Tiny steps → compounding results.', facing:'spine' },
    { id:'b_5', title:'Circe', author:'Madeline Miller', type:'fiction', rating:5, dateRead:'2019-07-08', cover:'', note:'Fierce, magical, woman-forward retelling.', facing:'spine' },
